...
# The 7th bit is the Busy indication bit. 1 = Busy, 0 = not.
AHT20_STATUSBIT_BUSY = 7
# The 3rd bit is the CAL (calibration) Enable bit. 1 = Calibrated, 0 = not
...
class AHT20:
    ...
    def cmd_soft_reset(self):
        ...
        with SMBus(self.BusNum) as i2c_bus:
            i2c_bus.write_i2c_block_data(
                AHT20_I2CADDR, 0x0, AHT20_CMD_SOFTRESET)
    ...
    def cmd_initialize(self):
        ...
        with SMBus(self.BusNum) as i2c_bus:
            i2c_bus.write_i2c_block_data(
                AHT20_I2CADDR, 0x0, AHT20_CMD_INITIALIZE)
        return True
    ...
    def get_measure_CRC8(self):
        """
        This function will calculate crc8 code with
        G(x) = x8 + x5 + x4 + 1 -> 0x131(0x31)
        Initial value = 0xFF
        No XOROUT.
        ...
        return: all_data (
            1 bytes status
            2.5 byes humidity
            2.5 bytes temperature
            1 bytes crc8 code), isCRC8_pass
        """
