...
# G(x) = x8 + x5 + x4 + 1 -> 0x131(0x31), Initial value = 0xFF. No XOROUT
# Author: XU Zifeng
# Email: zifeng.xu@foxmail.com

# PEP-8 format and spelling fixes - rlhatcher
...
# Divide number retrieve from CRC-8 MAXIM G(x) = x8 + x5 + x4 + 1
CRC_DIVIDE_NUMBER = 0x131
...
def mod2_division_8bits(a, b, number_of_bytes, init_value):
    "calculate mod2 division in 8 bits. a mod b. init_value is for crc8 init"
    head_of_a = 0x80
    # Processing a
    a = a << 8
...
def AHT20_crc8_calculate(all_data_int):
    ...
    mod_value = mod2_division_8bits(
        data_from_AHT20, CRC_DIVIDE_NUMBER, len(all_data_int), init_value)
    # print(mod_value)
    return mod_value
